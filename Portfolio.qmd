...
::: {.column width="66%"}
NEED TO MIDDLE ALIGN THIS

In online advertising, marketers strive to allocate their ad budgets efficiently -- investing at the right level in channels that perform, while avoiding diminishing returns relating to overspend. 

<br> Efficient budget allocation can make the difference between positive return on marketing investment, and a pile of wasted cash. To get it right, marketers need to know which channels are truly driving incremental performance. 

<br> Once-popular approaches like multi-touch and rules-based attribution have become increasingly less reliable, as the quality of user-level tracking across the web degrades. 

<br><b>The problem:</b> my client, a multinational universal bank, wanted to allocate their online ad budget in an evidence-based way, but did not have a reliable model to evaluate which channels were pulling the most weight. 
:::
...
<b>The Problem:</b> We decided to test the hypothesis that we were under-invested in paid search advertising. We needed a way to formally test that increased spend would deliver incremental approved credit cards, as the model suggested. 
 
<b>The Solution:</b> run a matched-markets test. This is a type of quasi-experiment, following this process:

- Two geographical markets that historically follow similar performance trends are identified
- Spending is increased (or, alternatively, can be paused) in only one of the two (the "test market")
- All else being equal, performance changes in the test market that differ significantly from the control market can be attributed to the change in spend
